...
/**
 * Некоторые классы заменил на свои, некоторые оставил из примера
 */
const basketCounterEl = document.querySelector('.page-header__btn-cart span');
 ...
 */
document.querySelector('.page-header__btn-cart').addEventListener('click', () => {
   event.preventDefault() // т.к. иконка у меня ссылка
...
document.querySelector('.products').addEventListener('click', event => {
  // Проверяем, если клик был не по кнопке, то ничего не делаем, т.к. у меня несколько элемнтов внутри кнопки, обращаюсь к их родители.
  ...
  const featuredItemEl = event.target.closest('.products-item');
...
/**
 * Функция `addToCart` добавляет продукт в корзину.
 ...
 * ! нужно добавить условие, если `number > 9`, то добавить style.right: '10px'.
 */
